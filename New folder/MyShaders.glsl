...
bool InFshape( vec2 pos ) {
	...
	if ( pos.x>= sideMargin && pos.x <= sideMargin+postWidth ) {
	    return true;
	}
	if ( pos.x>= 1.0 - sideMargin - postWidth && pos.x <= 1- sideMargin ) {
	    return true;
	}
	if(pos.y >= verticalMargin + 0.3 && pos.y<= 1 - verticalMargin - 0.3){
		return true;
	}
